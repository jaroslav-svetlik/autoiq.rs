...
class TrendBlogPostSeederTest extends TestCase
{
    ...
    public function test_trend_blog_post_seeder_creates_idempotent_unique_article_catalog(): void
    {
        ...
        $this->assertCount(540, $posts);
        $this->assertSame($posts->count(), $posts->pluck('slug')->unique()->count());
        $this->assertSame($posts->count(), $posts->pluck('title')->unique()->count());
        $this->assertSame(1, $posts->where('is_featured', true)->count());
        $this->assertCount(121, $posts->where('category', 'Poređenje modela'));
        ...
        $this->assertTrue($posts->contains('slug', 'nosac-opruge-na-polovnom-autu-kada-pukotina-trazi-proveru-trapa-i-korozije'));
        $this->assertTrue($posts->contains('slug', 'uvoz-auta-iz-jordana-kada-topla-klima-trazi-proveru-hladjenja-papira-i-stanja'));
        ...
        $this->assertTrue($posts->contains('slug', 'unutrasnji-retrovizor-na-polovnom-autu-kada-mali-detalj-trazi-proveru-stakla-kabine-i-bezbednosti'));
        $this->assertTrue($posts->contains('slug', 'uvoz-auta-iz-kirgistana-kada-udaljena-ruta-trazi-proveru-porekla-papira-i-stanja'));
        $this->assertTrue($posts->contains('slug', 'dodge-nitro-ili-jeep-patriot-suv-kada-pogon-i-sasija-moraju-pobediti-izgled'));
        $this->assertTrue($posts->contains('slug', 'polovni-infiniti-ex37-crossover-koji-mora-dokazati-motor-automatik-i-poreklo'));
        $this->assertTrue($posts->contains('slug', 'polovni-chevrolet-hhr-porodicni-karavan-koji-mora-dokazati-limariju-motor-i-delove'));
        $this->assertTrue($posts->contains('slug', 'zaptivka-gepeka-na-polovnom-autu-kada-gumeni-rub-trazi-proveru-vlage-i-zadnjeg-dela'));
        $this->assertTrue($posts->contains('slug', 'uvoz-auta-iz-uzbekistana-kada-udaljena-ruta-trazi-proveru-porekla-papira-i-stanja'));
        $this->assertTrue($posts->contains('slug', 'mazda-verisa-ili-daihatsu-cuore-mali-auto-kada-retkost-mora-dokazati-smisao'));
        $this->assertTrue($posts->contains('slug', 'polovni-ssangyong-tivoli-crossover-koji-mora-dokazati-motor-automatik-i-istoriju'));
        ...
        $this->assertTrue($posts->contains('slug', 'brava-gepeka-na-polovnom-autu-kada-tvrd-klik-otkriva-vlagu-udarac-ili-losu-popravku'));
        $this->assertTrue($posts->contains('slug', 'uvoz-auta-iz-severne-makedonije-kada-blizina-trazi-proveru-papira-porekla-i-ukupnog-racuna'));
