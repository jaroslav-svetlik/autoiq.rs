    ...
    @if ($paginator->hasPages())
        <nav role="navigation" aria-label="Paginacija" class="panel-soft flex flex-col gap-4 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
            <div class="text-sm leading-6 text-slate-400">
                ...
                od
                <span class="font-semibold text-cyan-200">{{ number_format($paginator->total(), 0, ',', '.') }}</span>
                rezultata
            </div>
                            ...
                            @foreach ($element as $page => $url)
                                <span wire:key="paginator-{{ $paginator->getPageName() }}-page{{ $page }}">
                                    @if ($page == $paginator->currentPage())
                                        <span aria-current="page" class="inline-flex h-10 min-w-10 items-center justify-center rounded-xl bg-cyan-300 px-3 text-sm font-bold text-slate-950 shadow-[0_10px_28px_rgba(14,165,233,0.28)]">
                                            {{ $page }}
                                        </span>
                                    @else
                                        <button type="button" wire:click="gotoPage({{ $page }}, '{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" wire:loading.attr="disabled" class="inline-flex h-10 min-w-10 items-center justify-center rounded-xl px-3 text-sm font-semibold text-slate-300 transition hover:bg-white/10 hover:text-white focus:outline-none focus:ring-2 focus:ring-cyan-300/30 disabled:cursor-not-allowed disabled:opacity-60" aria-label="Idi na stranu {{ $page }}">
                                            {{ $page }}
                                        </button>
                                    @endif
                                </span>
